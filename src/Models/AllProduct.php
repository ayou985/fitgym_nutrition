...
class AllProduct
{
    ...
    public static function getReviewsByProductId($productId)
    {
        ...
        $query = $db->prepare("
            SELECT r.id, r.comment, r.rating, r.created_at, r.user_id, u.username, u.lastname
            FROM reviews r
    ...
    {
        $pdo = \Config\Database::getInstance()->getConnection();
        $stmt = $pdo->prepare("SELECT * FROM reviews WHERE user_id = :user_id AND product_id = :product_id LIMIT 1");
        $stmt->execute([
            'user_id' => $userId,
            'product_id' => $productId
        ]);
